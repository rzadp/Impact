...
// exact copy as igFont but extends tpfImage instead of extending igImage
export class igFontTpf extends tpfImage {
	widthMap= [];
	indices= [];
	firstChar= 32;
	alpha= 1;
  ...
  static ALIGN = {
	...
	draw( text, x, y, align ) {
		...
		if( align == igFontTpf.ALIGN.RIGHT || align == igFontTpf.ALIGN.CENTER ) {
			var width = this._widthForLine( text );
			x -= align == igFontTpf.ALIGN.CENTER ? width/2 : width;
		}
		...
		tpfImage.drawCount += text.length;
	}
