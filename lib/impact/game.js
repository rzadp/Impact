...
export class igGame {
	...
	namedEntities= {};
	collisionMap= igCollisionMap.staticNoCollision;
	backgroundMaps= [];
	backgroundAnims= {};
	...
	loadLevel( data ) {
		...
		for( var i = 0; i < data.layer.length; i++ ) {
			...
			if( ld.name == 'collision' ) {
				...
				var newMap = new igBackgroundMap(ld.tilesize, ld.data, ld.tilesetName);
  ...
  getEntityClass(type) {
    ...
    ? ig.global[type]
    : type;
    if (!entityClass) throw new Error('Cannot find entity class. Override class selection in the overriding game class.')
    return entityClass
  }
	
	
	getEntitiesByType( type ) {
		var entityClass = this.getEntityClass(type)
    ...
    console.log('type', type, typeof(type))
		...
		if( !entityClass ) {
			throw("Can't spawn entity of type " + type + '. Override getEntityClass in the game class.');
		}
	...
	removeEntity( ent ) {
		...
		// updated or checked
		ent._killed = true;
		ent.type = igEntity.TYPE.NONE;
		ent.checkAgainst = igEntity.TYPE.NONE;
		ent.collides = igEntity.COLLIDES.NEVER;
	...
	checkEntities() {
		...
		for( var e = 0; e < this.entities.length; e++ ) {
			...
			if(
				entity.type == igEntity.TYPE.NONE &&
				entity.checkAgainst == igEntity.TYPE.NONE &&
				entity.collides == igEntity.COLLIDES.NEVER
			) {
				continue;
			}
