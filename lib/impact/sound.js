...
export class igSoundManager {
	...
	loadWebAudio( path, multiChannel, loadCallback ) {
		...
		var audioSource = new igSoundWebAudioSource();
	...
	loadHTML5Audio( path, multiChannel, loadCallback ) {
		...
		if( this.clips[path] ) {
			...
			if( this.clips[path] instanceof igSoundWebAudioSource ) {
				return this.clips[path];
			}
	...
	get( path ) {
		...
		if( channels && channels instanceof igSoundWebAudioSource ) {
			return channels;
		}
...
export class igMusic {
	...
	add( music, name ) {
		...
		if( track instanceof igSoundWebAudioSource ) {
