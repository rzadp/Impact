export class igSoundManager {
	...
	init() {
		...
		var probe = new Audio();
		for( var i = 0; i < igSound.use.length; i++ ) {
			var format = igSound.use[i];
	...
	loadWebAudio( path, multiChannel, loadCallback ) {
		...
		var audioSource = new igSound.WebAudioSource();
	...
	loadHTML5Audio( path, multiChannel, loadCallback ) {
		...
		if( this.clips[path] ) {
			...
			if( this.clips[path] instanceof igSound.WebAudioSource ) {
				return this.clips[path];
			}
			
			// Only loaded as single channel and now requested as multichannel?
			if( multiChannel && this.clips[path].length < igSound.channels ) {
				for( var i = this.clips[path].length; i < igSound.channels; i++ ) {
		...
		if( multiChannel ) {
			for( var i = 1; i < igSound.channels; i++ ) {
				var a = new Audio(realPath);
				a.load();
				this.clips[path].push( a );
			}
	...
	get( path ) {
		...
		if( channels && channels instanceof igSound.WebAudioSource ) {
			return channels;
		}
...
export class igMusic {
	...
	add( music, name ) {
		...
		var path = music instanceof igSound ? music.path : music;
		...
		if( track instanceof igSound.WebAudioSource ) {
...
export class igSoundWebAudioSource {
	sources= [];
	gain= null;
	buffer= null;
	_loop= false;
