...
export class igSystem {
  ...
  static scaleMode = igSystem.SCALE.SMOOTH;
	...
	constructor( canvasId, fps, width, height, scale, contextOverride ) {
		this.fps = fps;
		
    this.clock = new igTimer();
    console.log('canvasId', canvasId)
    this.canvas = ig.$(canvasId);
    assert(this.canvas, 'Canvas not there')
		this.resize( width, height, scale );
		this.context = contextOverride || this.canvas.getContext('2d');
